...
def user_exceeds_limits(request, top_level=False):
    """
    TODO: Puts on limits on how many posts a user can post.
    """
    ...
    #     comment = POST_LIMIT_ERROR_MSG % max_post_limit
    #     logger.error("post limit reached for %s, %s, %s", user, comment, request)
    #     return True

    # # This only needs to be checked when creating top level post
    # if top_level and ((top_post_count + 1) > max_top_post_limit):
    #     comment = TOP_POST_LIMIT_ERROR_MSG % max_top_post_limit
    #     logger.error("top post limit reached for %s, %s, %s", user, comment, request)
    #     return True
...
class PostModeration(LoginRequiredMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not post.is_editable:
            logger.warning("TODO: You may not moderate this post (Request: %s)", request)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not form.is_valid():
            logger.error("%s, %s", form.errors, request)
            return response
        ...
        if action == (BUMP_POST) and post != post.root:
            logger.error("Only top-level posts may be bumped! (Request: %s)", request)
            return response
        ...
        if action == (OPEN, TOGGLE_ACCEPT):
            logger.error("Only a moderator may open or toggle a post (Request: %s)", request)
            return response
        ...
        if action in (CLOSE_OFFTOPIC, DUPLICATE) and not post.is_toplevel:
            logger.warning("You can only close or open a top level post (Request: %s)", request)
            return response

        if action == OPEN:
            query.update(status=Post.OPEN)
            logger.info("Opened post: %s (Request: %s)", post.title, request)
            return response
        ...
        if action == DELETE:
            ...
            if delete_only:
                ...
                logger.info("Deleted post: %s (Request: %s)", post.title, request)
                response = HttpResponseRedirect(post.root.get_absolute_url())
            else:
                ...
                post.delete()
                logger.info("Removed post: %s (Request: %s)", post.title, request)
        ...
        logger.warning("That seems to be an invalid action for that post. \
                It is probably ok! Actions may be shown even when not valid.  (Request: %s)", request)
        return response
...
class UserModeration(LoginRequiredMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if target.is_administrator:
            logger.warning("Cannot moderate an administrator (Request: %s)", request)
            return response
        ...
        if not target.is_editable:
            logger.warning("Target not editable by this user (Request: %s)", request)
            return response

        form = self.form_class(request.POST, pk=target.id)
        if not form.is_valid():
            logger.error("Invalid user modification action (Request: %s)", request)
            return response
